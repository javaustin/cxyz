...
import static com.carrotguy69.cxyz.other.ObjectUtils.removeFromMultimap;
    ...
    public static void handleNetworkPlayerDelivery(String json) {
        ...
        try {
            ...
            for (NetworkPlayer np : wrapper.getDeletedData()) {
                Logger.info("[-] Deleted an entry of old data from users! " + np.getUUID());
    ...
    public static void handlePartyDelivery(String json) {
        ...
        try {
            ...
            for (Party party : wrapper.getDeletedData()) {
                Logger.info("[-] Deleted an entry of old data from parties! " + party.toString());
    ...
    public static void handlePartyInviteDelivery(String json) {
        ...
        try {
            ...
            for (PartyInvite invite : wrapper.getDeletedData()) {
                Logger.info("[-] Deleted an entry of old data from partyInvites! " + invite.toString());

                removeFromMultimap(partyInvites, invite.getInviterUUID(), invite);
            }

            for (PartyInvite invite : wrapper.getNewData()) {
                if (partyInvites.containsKey(invite.getInviterUUID()) && partyInvites.containsValue(invite)) {
                    // The exact same invite is already in our map. We want to avoid duplicates because this MultiMap will actually allow them if we don't check before.
    ...
    public static void handlePartyExpireDelivery(String json) {
        ...
        try {
            ...
            for (PartyExpire expire : wrapper.getDeletedData()) {
                Logger.info("[-] Deleted an entry of old data from partyExpires! " + expire.toString());
    ...
    public static void handlePunishmentDelivery(String json) {
        ...
        try {
            ...
            for (Punishment punishment : wrapper.getDeletedData()) {
                Logger.info("[-] Deleted an entry of old data from punishments! " + punishment.toString());
    ...
    public static void handleMessageDelivery(String json) {
        ...
        try {
            ...
            for (Message msg : wrapper.getDeletedData()) {
                Logger.info("[-] Deleted an entry of old data from messages! " + msg.toString());
            ...
            for (Message msg : wrapper.getNewData()) {
                ...
                if (messageMap.containsKey(msg.getRecipientUUID()) && messageMap.containsValue(msg)) {
                    // The exact same invite is already in our map. We want to avoid duplicates because this MultiMap will actually allow them if we don't check before.
                    Logger.info("[~] Ignored duplicate entry from messages! " + msg);
                    continue;
                }

                Logger.info("[+] Added/Modified an entry to messages. " + msg);
    ...
    public static void handleFriendRequestDelivery(String json) {
        ...
        try {
            ...
            for (FriendRequest request : wrapper.getDeletedData()) {
                Logger.info("[-] Deleted an entry of old data from friendRequests! " + request.toString());

                removeFromMultimap(friendRequests, request.getSenderUUID(), request);
            }

            for (FriendRequest request : wrapper.getNewData()) {
                if (friendRequests.containsKey(request.getSenderUUID()) && friendRequests.containsValue(request)) {
                    // The exact same invite is already in our map. We want to avoid duplicates because this MultiMap will actually allow them if we don't check before.
