package com.carrotguy69.cxyz.cmd.party;
...
public class PartyPrivate implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String s, @NotNull String[] args) {
        ...
        */
        ...
        String node = "cxyz.party.private";
    ...
    public void setPrivate(NetworkPlayer np) {
        ...
        if (!party.isPublic()) {
            MessageUtils.sendParsedMessage(np.getPlayer(), MessageKey.PARTY_ERROR_ALREADY_PRIVATE, MapFormatters.partyFormatter(party));
            return;
        }

        party.setPublic(false);

        MessageUtils.sendParsedMessage(np.getPlayer(), MessageKey.PARTY_SET_PUBLIC_FALSE, MapFormatters.partyFormatter(party));
        ...
        Collection<PartyInvite> invites = partyInvites.get(np.getUUID());
        List<PartyInvite> toDelete = new ArrayList<>();

        for (PartyInvite invite : invites) {
            if (invite != null && Objects.equals(invite.getInviterUUID(), np.getUUID())) {
        ...
        partyInvites.removeAll(np.getUUID()); // Remove all invites from the inviter when we are done sending requests.
