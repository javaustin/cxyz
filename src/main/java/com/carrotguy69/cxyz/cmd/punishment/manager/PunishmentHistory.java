...
public class PunishmentHistory implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String s, @NotNull String[] args) {
        ...
        */
        ...
        String node = "cxyz.punishment.history";
    ...
    private void viewHistoryAll(CommandSender sender, int page) {
        List<Punishment> punishments = Punishment.getAllPunishments();
        punishments.sort(Comparator.comparing(Punishment::getIssuedTimestamp));

        Map<String, Object> commonMap = new HashMap<>();
        String format = MessageGrabber.grab(MessageKey.PUNISHMENT_HISTORY_ALL_FORMAT);
        String delimiter = MessageGrabber.grab(MessageKey.PUNISHMENT_HISTORY_ALL_SEPARATOR);

        if (punishments.isEmpty()) {
            MessageUtils.sendParsedMessage(sender, MessageKey.PUNISHMENT_HISTORY_ALL_LIST_NONE, commonMap);
            return;
        }

        int maxEntriesPerPage = msgYML.getInt(MessageKey.PUNISHMENT_HISTORY_ALL_MAX_ENTRIES.getPath(), -1);
        ...
        String unparsed = MessageGrabber.grab(MessageKey.PUNISHMENT_HISTORY_ALL_LIST);
