...
public class CosmeticBuy implements CommandExecutor {
    @Override
    ...
    public void buy(Player p, String cosmeticID) {
        ...
        if (!cosmetic.isEnabled()) {
            MessageUtils.sendParsedMessage(p, MessageKey.COSMETIC_BUY_ERROR_DISABLED_ITEM, commonMap);
            return;
        }

        if (np.hasOwnedCosmetic(cosmetic)) {
            MessageUtils.sendParsedMessage(p, MessageKey.COSMETIC_BUY_ERROR_DUPLICATE_ITEM, commonMap);
            return;
        }
        ...
        if (rank.getHierarchy() < cosmetic.getRequiredRank().getHierarchy()) {
            ...
            MessageUtils.sendParsedMessage(p, MessageKey.COSMETIC_BUY_ERROR_INSUFFICIENT_LEVEL, commonMap);
            return;
        }

        else if (coins < cosmetic.getPrice()) {
            MessageUtils.sendParsedMessage(p, MessageKey.COSMETIC_BUY_ERROR_INSUFFICIENT_COINS, commonMap);
            return;
        }
        ...
        MessageUtils.sendParsedMessage(p, MessageKey.COSMETIC_BUY_SUCCESS, commonMap);
    }
