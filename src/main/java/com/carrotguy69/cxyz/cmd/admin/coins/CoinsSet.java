...
public class CoinsSet implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String s, @NotNull String[] args) {
        ...
        String node = "cxyz.admin.coins.set";
        ...
        //                (-1)    0       1
        // Syntax: /coins set {amount} {player}
    ...
    public static void set(CommandSender sender, NetworkPlayer np, long amt) {
        ...
        np.setCoins(absoluteAmount);
        np.sync();

        Map<String, Object> formatted = MapFormatters.playerFormatter(np);
        formatted.put("coins", absoluteAmount);
        formatted.put("amount", absoluteAmount);

        MessageUtils.sendParsedMessage(sender, MessageKey.COINS_SET, formatted);
