...
import com.carrotguy69.cxyz.classes.models.config.*;
...
public class NetworkPlayer {
    ...
    public PlayerRank getRank() {
        try {
            return PlayerRank.getRankByName(rank);
        }
    ...
    public boolean isVisibleTo(NetworkPlayer np) {
        ...
        if (this.isVanished() && this.getRank().getHierarchy() > np.getRank().getHierarchy()) {
            return false;
        }
    ...
    public void sync() {
        Requests.postRequest(api_endpoint + "/user/modify", gson.toJson(this));
    }

    public void create() {
        Requests.postRequest(api_endpoint + "/user/create", gson.toJson(this));
    }
    ...
    public boolean sendUnparsedMessage(String content, Map<String, Object> formatMap) {
        ...
        if (Objects.equals(this.getServer().getName(), this_server.getName())) {
            ...
            Requests.postRequest(Constants.getGameServerIP(this.getServer()) + "/sendMessage", gson.toJson(
                    Map.of(
                            "recipientUUID", uuid,
                            "content", String.join("\n", content),
                            "parsed", false,
    ...
    public boolean sendParsedMessage(String content, Map<String, Object> formatMap) {
        ...
        if (Objects.equals(this.getServer().getName(), this_server.getName())) {
            ...
            MessageUtils.sendParsedMessage(this.getPlayer(), content, formatMap);
        }
    ...
    public void kick(String reason) {
        ...
        if (Objects.equals(this.getServer().getName(), CXYZ.this_server.getName())) {
            ...
            Requests.postRequest(Constants.getGameServerIP(this.getServer()) + "/kickPlayer", gson.toJson(
                    Map.of(
