...
import static com.carrotguy69.cxyz.other.messages.MessageUtils.forceColor;
...
public class MessageParser {
    ...
    public class MessageParseException extends RuntimeException {
        ...
        public MessageParseException(String description, int index) {
            ...
            Logger.severe(description + "At: " + index + "\n" + "With original text: \"" + unparsed + "\"");
        }
    ...
    public boolean isInteractive() {
        for (SimpleTextComponent textComponent : safeParse()) {
    ...
    public List<SimpleTextComponent> parse() throws MessageParseException {
        ...
        for (int i = 0; i < unparsed.length(); i++) {
            ...
            if (character == '(' && nextCharacter != '(') { // Opening character (Not doubled/escaped)
                ...
                for (int j = openingParenthesis; j < unparsed.length(); j++) { // Immediately find closing character
                    ...
                    if (j != i && unparsed.charAt(j) == '(' && (j + 1 == unparsed.length() || unparsed.charAt(j + 1) != '(')) { // There is an unexpected parenthesis that is not escaped! Error!
                        throw new MessageParseException(
                                String.format("Unexpected '(' at index %d. If you intended a literal '(', escape it by doubling: ((", i)
                        );
                    }

                    if (unparsed.charAt(j) == ')' && (j + 1 == unparsed.length() || unparsed.charAt(j + 1) != ')')) { // Closing character (Not doubled/escaped)
                ...
                SimpleTextComponent textComponent = new SimpleTextComponent(
                        formatPlaceholders(textSubstring, formatMap),
                        new ArrayList<>()
                );
                ...
                while (true) {
                    if (start + 1 > unparsed.length()) {
                        ...
                        if (closingBracket == -1) {// No matching closing bracket found for this opening bracket
                            throw new MessageParseException(String.format("Expected closing bracket ']' to match opening bracket at index %d in action block!", openingBracket));
                        }
                        ...
                        try {
                            ...
                            SimpleTextComponent.Action action = new SimpleTextComponent.Action(type, formatPlaceholders(line, formatMap));
            ...
            // We can throw errors here for fun. Even though the parser can work through it, it is better to enforce consistency.

            // Everything below this line assumes we are outside of text or action blocks


            else if (character == '[' && nextCharacter == '[' || character == ']' && nextCharacter == ']' || character == '(' || character == ')' && nextCharacter == ')') { // We want to be sure we are escaping doubled up brackets.
                ...
                throw new MessageParseException(
                        String.format("Unexpected '[' at index %d. Brackets must directly follow a text block (e.g. (text)[ACTION]). If you intended a literal '[', escape it by doubling: [[", i)
                );
            }
                ...
                throw new MessageParseException(
                        String.format("Unexpected ')' at index %d. Closing parentheses must match an opening '(' that starts a text block. If you intended a literal ')', escape it by doubling: ))", i)
                );
            }
        ...
        if (!stringBuilder.toString().isEmpty()) {
            components.add(new SimpleTextComponent(formatPlaceholders(stringBuilder.toString(), formatMap), List.of()));
        }
    ...
    public List<SimpleTextComponent> safeParse() throws MessageParseException {
        List<SimpleTextComponent> components = new ArrayList<>();
        StringBuilder stringBuilder = new StringBuilder();
        ...
        for (int i = 0; i < unparsed.length(); i++) {
            ...
            // Everything below this line assumes we are outside of text or action blocks
                ...
                Logger.warning(String.format("Unexpected '[' at index %d. Brackets must directly follow a text block (e.g. (text)[ACTION]). If you intended a literal '[', escape it by doubling: [[", i));
                invalid = true;
                index = i;
                break;
//                throw new MessageParseException(
...
//                );
                ...
                Logger.warning(String.format("Unexpected ')' at index %d. Closing parentheses must match an opening '(' that starts a text block. If you intended a literal ')', escape it by doubling: ))", i));
                invalid = true;
                index = i;
                break;
...
//                );
        ...
        if (invalid) {
            stringBuilder.append(unparsed, index, unparsed.length());
        }
        ...
        Logger.log(components.toString());
        ...
        try {
            ...
            for (Map.Entry<SimpleTextComponent, ChatColor> entry : colorSTCMap.entrySet()) {
                ...
                if (simple.getActions().isEmpty()) {
                    TextComponent component = new TextComponent(f(forceColor(simple.getContent())));
